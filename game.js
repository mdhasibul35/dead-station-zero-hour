...
class Game {
    ...
    setupEvents() {
        ...
        window.addEventListener('mousedown', (e) => {
            ...
            // Strict touch isolation: NEVER register mouse down if on a touch device or synthetic event
            if (this.isTouchDevice || ('ontouchstart' in window) || (e.sourceCapabilities && e.sourceCapabilities.firesTouchEvents)) {
                this.isMouseDown = false;
                return;
            }
    ...
    fireWeaponMobileAuto() {
        ...
        // Toned down aim assist: fire forward with only subtle guidance if an enemy is in front sights
        let forwardTarget = null;
        let bestScore = -1;
        const forward = new THREE.Vector3(Math.sin(p.mesh.rotation.y), 0, Math.cos(p.mesh.rotation.y)).normalize();

        for (let a of this.alienMgr.aliens) {
            if (!a.alive) continue;
            const toAlien = new THREE.Vector3().subVectors(a.mesh.position, p.mesh.position);
            toAlien.y = 0;
            const dist = toAlien.length();
            // Close/medium range only (under 16 units)
            if (dist < 16 && dist > 0.1) {
                const dot = forward.dot(toAlien.clone().normalize());
                // Only targets directly in front within a narrow forward cone (dot > 0.85, ~30 degrees)
                if (dot > 0.85) {
        ...
        if (forwardTarget) {
            // Gentle subtle rotation nudge towards target in front
            const toTarget = new THREE.Vector3().subVectors(forwardTarget.mesh.position, p.mesh.position);
            toTarget.y = 0;
            const targetRot = Math.atan2(toTarget.x, toTarget.z);
            p.mesh.rotation.y = THREE.MathUtils.lerp(p.mesh.rotation.y, targetRot, 0.4);
            this.mouseWorld.copy(forwardTarget.mesh.position);
        } else {
            // Pure manual firing: shoot straight in operative's facing direction
            this.mouseWorld.set(
                p.mesh.position.x + forward.x * 25,
                0,
                p.mesh.position.z + forward.z * 25
            );
        }
    ...
    updatePlayer(dt) {
        ...
        if (this.isTouchDevice) {
            // ---------------------------------------------------------------
            // MOBILE TOUCH CONTROL SYSTEM (STRICTLY MANUAL, ZERO AUTO FIRING)
            // ---------------------------------------------------------------
            if (this.isScreenFiring) {
                // Manual screen touch held on right side: aim at touch point and fire
                this.raycaster.setFromCamera(this.screenTouchAim, this.camera);
                const hit = new THREE.Vector3();
                if (this.raycaster.ray.intersectPlane(this.groundPlane, hit)) {
                    this.mouseWorld.copy(hit);
                ...
                // Manual FIRE button held: fire in aim direction (with gentle forward assist)
                if (p.ammo[p.currentWeapon.id] <= 0) {
                    this.reloadCurrentWeapon();
                } else {
                    this.fireWeaponMobileAuto();
                ...
                // Moving operative with left joystick: face movement heading. STRICTLY ZERO FIRING!
                ...
                // IDLE / STANDING STILL ON MOBILE: STRICTLY ZERO FIRING!
                // Operative stands still and awaits touch command. ZERO FIRING!
            }
        } else {
            // ---------------------------------------------------------------
            // DESKTOP PC / WEB SYSTEM
            // ---------------------------------------------------------------
            // Desktop mouse raycasting onto ground plane defines facing direction
            this.raycaster.setFromCamera(this.mousePos, this.camera);
            const hit = new THREE.Vector3();
            if (this.raycaster.ray.intersectPlane(this.groundPlane, hit)) {
                this.mouseWorld.copy(hit);
                ...
                toMouse.y = 0;
                if (toMouse.lengthSq() > 0.01) {
                    p.mesh.rotation.y = Math.atan2(toMouse.x, toMouse.z);
                }
            }
    ...
    updateHUD() {
        ...
        if (wName) wName.textContent = p.currentWeapon.name;
